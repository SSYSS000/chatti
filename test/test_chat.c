...
static void test_chat_message(void)
{
    ...
    // To network
    rc = chat_message_to_network(&cm, buffer, 1);
    EXPECT_TRUE(rc < 0, "Expected buffer to be too small\n");

    rc = chat_message_to_network(&cm, buffer, 10);
    EXPECT_TRUE(rc < 0, "Expected buffer to be too small\n");

    rc = chat_message_to_network(&cm, buffer, sizeof buffer);
    ...
    // From network
    rc = network_to_chat_message(&cm, buffer, 1);
    EXPECT_TRUE(rc < 0, "Expected error for corrupt message\n");

    rc = network_to_chat_message(&cm, buffer, 10);
    EXPECT_TRUE(rc < 0, "Expected error for corrupt message\n");

    rc = network_to_chat_message(&cm, buffer, EXPECTED_NET_CM_LEN);
    EXPECT_TRUE(rc > 0, "Expected successful conversion\n");
    EXPECT_TRUE(rc == EXPECTED_NET_CM_LEN, "Incorrect converted length (%d), expected %d\n", rc, EXPECTED_NET_CM_LEN);
...
void test_chat_member_join(void)
{
    ...
    // To network
    rc = chat_member_join_to_network(&cm, buffer, 1);
    EXPECT_TRUE(rc < 0, "Expected buffer to be too small\n");

    rc = chat_member_join_to_network(&cm, buffer, sizeof buffer);
    EXPECT_TRUE(rc > 0, "Expected successful conversion\n");
    ...
    // From network
    rc = network_to_chat_member_join(&cm, buffer, 1);
    EXPECT_TRUE(rc < 0, "Expected error for corrupt message\n");

    rc = network_to_chat_member_join(&cm, buffer, EXPECTED_NET_CMJ_LEN);
    EXPECT_TRUE(rc > 0, "Expected successful conversion\n");
    EXPECT_TRUE(rc == EXPECTED_NET_CMJ_LEN, "Incorrect converted length (%d), expected %d\n", rc, EXPECTED_NET_CMJ_LEN);
    EXPECT_TRUE(!strcmp(cm.sender, SENDER), "Converted data is invalid\n");
}
...
void test_chat_member_leave(void)
{
    ...
    // To network
    rc = chat_member_leave_to_network(&cm, buffer, 1);
    EXPECT_TRUE(rc < 0, "Expected buffer to be too small\n");

    rc = chat_member_leave_to_network(&cm, buffer, sizeof buffer);
    EXPECT_TRUE(rc > 0, "Expected successful conversion\n");
    ...
    // From network
    rc = network_to_chat_member_leave(&cm, buffer, 1);
    EXPECT_TRUE(rc < 0, "Expected error for corrupt message\n");

    rc = network_to_chat_member_leave(&cm, buffer, EXPECTED_NET_CML_LEN);
    EXPECT_TRUE(rc > 0, "Expected successful conversion\n");
    EXPECT_TRUE(rc == EXPECTED_NET_CML_LEN, "Incorrect converted length (%d), expected %d\n", rc, EXPECTED_NET_CML_LEN);
    EXPECT_TRUE(!strcmp(cm.sender, SENDER), "Converted data is invalid\n");
}
